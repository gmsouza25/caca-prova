...
**Setup de deploy em uso (30/08, conta `294d134330d5b019e048675a56cc8b84`):**
- Credenciais em `.env.deploy` (gitignored): `CLOUDFLARE_ACCOUNT_ID` + `CLOUDFLARE_API_TOKEN`
  (permissão `Account→Cloudflare Pages→Edit`).
- Se o token der `401/403 "Cannot use the access token from location"` → é **filtro de IP**;
  em `My Profile→API Tokens→Edit`, mude "Client IP Address Filtering" para **Any IP address**.
- O `wrangler` máximo aqui é **v3.112** (o mais novo exige Node 22; o sandbox tem Node 20):
  `npm i --no-save wrangler@3.112.0`. Reinstalar a cada sessão (node_modules não persiste).
- Publicação de produção: `./node_modules/.bin/wrangler pages deploy dist --project-name="caca-prova" --branch main --commit-dirty=true`.
  (O `deploy-pages.sh` publica a pasta `dist/`; `dist` NÃO persiste — rodar `build-config`+`build-pages` antes.)
